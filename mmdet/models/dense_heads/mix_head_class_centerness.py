...
class MixHead(nn.Module):
    ...
    def loss(self,
        ...
        loss_AB=self.anchor_base.loss(cls_scores_AB,bbox_preds_AB,gt_bboxes,gt_labels,img_metas)
        ...
        if num_pos > 0:
            ...
            loss_bbox_AF = pos_bbox_preds.sum()
            loss_centerness = pos_centerness.sum()
        loss_cls_AB=0
        loss_bbox_AB=0
        for lc in loss_AB['loss_cls']:
            loss_cls_AB=loss_cls_AB+lc
        for lb in loss_AB['loss_bbox']:
            loss_bbox_AB=loss_bbox_AB+lb
        loss_bbox_AB=loss_bbox_AB/len(loss_AB['loss_bbox'])
        loss_cls_AB=loss_cls_AB/len(loss_AB['loss_cls'])
        loss_cls = loss_cls_AB + loss_cls_AF
        loss_bbox= loss_bbox_AB + loss_bbox_AF
        return dict(
            loss_cls=loss_cls,
            loss_bbox=loss_bbox,
