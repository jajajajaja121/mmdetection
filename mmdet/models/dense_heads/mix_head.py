...
INF = 1e8
class anchorBase_retina(RetinaHead):#继承retina的head
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
...
class anchorBase(AnchorHead):#继承anchor base类型,按照scales=[8,16.32]的大小来产生anchor
    def __init__(self,
                 num_classes,
                 in_channels,
                 anchor_generator=dict(
    ...
    def anchor_single(self, bbox_pred, anchors,
                    bbox_targets, bbox_weights, ):#获得单个特征层的anchor
        # classification loss
        # labels = labels.reshape(-1)
        # label_weights = label_weights.reshape(-1)
        # cls_score = cls_score.permute(0, 2, 3,
    ...
    def get_all_anchor(self,#获得所有特征层的anchor
             cls_scores,
             bbox_preds,
             gt_bboxes,
             gt_labels,
...
class MixHead(nn.Module):#将两种head融合到一起产生的head,
    ...
    def __init__(self,
        ...
        self.anchor_base=anchorBase_retina(#初始化anchor base的head
                 num_classes,
                 in_channels,
                 anchor_generator=anchor_generator,
                 reg_decoded_bbox=True,
    ...
    def init_weights(self):#初始化各个层的权重
    ...
    def forward_single(self, x, scale):
        ...
        cls_feat_AB=x
        reg_feat_AB=x
        for cls_conv in self.anchor_base.cls_convs:#anchor base的分类分支
            cls_feat_AB = cls_conv(cls_feat_AB)
        for reg_conv in self.anchor_base.reg_convs:#anchor base的回归分支
            reg_feat_AB = reg_conv(reg_feat_AB)
        cls_score_AB = self.anchor_base.retina_cls(cls_feat_AB)#得到分类分支的预测结果
        bbox_pred_AB = self.anchor_base.retina_reg(reg_feat_AB)#得到回归分支的预测结果
        # cls_score_AB = self.anchor_base.conv_cls(x)
        # bbox_pred_AB = self.anchor_base.conv_reg(x)

        for cls_layer in self.cls_convs:#anchor free的分类分支
            cls_feat = cls_layer(cls_feat)
        cls_score_AF = self.fcos_cls(cls_feat)#得到anchor free的分类结果
        centerness = self.fcos_centerness(cls_feat)#得到centerness的预测结果,这个参数越靠近GT的中心越接近1,
        ...
        bbox_pred_AF = scale(self.fcos_reg(reg_feat)).float().exp()#得到anchor free分支的预测结果
        cls_score=[cls_score_AB,cls_score_AF]#将两种head的分类得分打包
    ...
    def loss(self,
        ...
        bbox_pred,bbox_target,loss_cls_AB=self.anchor_base.get_all_anchor(#计算anchor base分支的分类损失
            cls_scores_AB,
            bbox_preds_AB,
            gt_bboxes,
            gt_labels,
            img_metas,)#获得anchor base的框,以及回归目标
        loss_cls_AF = self.loss_cls(#计算anchor free分支的分类损失
        ...
        if num_pos > 0:
            ...
            if bbox_pred.shape[0] <= 3000:#避免框的数量太多爆显存
                ...
                if bbox_pred.shape[0] > 0:#当有iou超过0.2的框时将anchor base的那些框和anchor free的框合并到一起
            ...
            loss_bbox = self.loss_bbox(#计算回归损失
                bbox_pred,
                bbox_target,
            ...
            loss_centerness = self.loss_centerness(pos_centerness,#计算centerness的损失
    ...
    def get_bboxes(self,
        ...
        result_list=self.anchor_base.get_bboxes(cls_scores_AB,bbox_preds_AB,img_metas,cfg,rescale)
        # assert len(cls_scores) == len(bbox_preds)
        # device = cls_scores[0][0].device
        # num_levels = len(cls_scores)
        # cls_scores_AB = [cls_score[0] for cls_score in cls_scores]#得到anchor base的分类得分
        # cls_scores_AF = [cls_score[1] for cls_score in cls_scores]#得到anchor free的分类得分
        # bbox_preds_AB = [bbox_pred[0] for bbox_pred in bbox_preds]#得到anchor base的回归得分
        # bbox_preds_AF = [bbox_pred[1] for bbox_pred in bbox_preds]#得到anchor free的回归得分
        # featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores_AF]#得到特征图的尺寸
        # mlvl_points = self.get_points(featmap_sizes, bbox_preds_AF[0].dtype,
        #                               bbox_preds_AF[0].device)
        # result_list = []
        # featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores_AF]#得到各级特征图的尺寸
        # anchors = self.anchor_base.anchor_generator.grid_anchors(#计算anchor,返回值为各级特征上anchor的坐标
        #     featmap_sizes, device=device)
        # for img_id in range(len(img_metas)):
        #     cls_score_list_AF = [
        #         cls_scores_AF[i][img_id].detach() for i in range(num_levels)#将一个batch图片的分类得分放到一起
        #     ]
        #     cls_score_list_AB = [
        #         cls_scores_AB[i][img_id].detach() for i in range(num_levels)
        #     ]
        #     cls_score_list=[cls_score_list_AF,cls_score_list_AB]#两种head的分类得分打包
        #     bbox_pred_list_AF = [
        ...
        #     ]
        #     bbox_pred_list=[bbox_pred_list_AF,bbox_pred_list_AB]#两种head的回归得分打包
        #     centerness_pred_list_AF = [
        #         centernesses[i][img_id].detach() for i in range(num_levels)#将一个batch的centerness放到一起
        #     ]
        #     centerness_pred_list_AB=[
        #         centernesses[i][img_id].repeat(9,1,1).detach() for i in range(num_levels)
        #     ]
        #     centerness_pred_list=[centerness_pred_list_AF,centerness_pred_list_AB]#将两种head的centerness放到一起
        ...
        #     img_shape = img_metas[img_id]['img_shape']#图片尺寸
        #     scale_factor = img_metas[img_id]['scale_factor']
        #     det_bboxes,det_labels = self._get_bboxes_single(cls_score_list,#得到最终的bbox预测结果
        #                                          bbox_pred_list,
        #                                          centerness_pred_list,
        #                                          mlvl_points, img_shape,
        #                                          scale_factor, cfg, anchors,rescale)
    ...
    def _get_bboxes_single(self,
        ...
        assert len(cls_scores[0]) == len(bbox_preds[0]) == len(mlvl_points)
        mlvl_bboxes = []
        mlvl_scores = []
        mlvl_centerness = []
        for cls_score, bbox_pred, centerness, points in zip(
                cls_scores[0], bbox_preds[0], centernesses[0], mlvl_points):#这个for循环得到anchor free分支的预测结果
        ...
        for cls_score_AB, bbox_pred_AB, points,anchor,centerness_AB in zip(#这个for循环得到anchor base分支的预测结果
                cls_scores[1], bbox_preds[1], mlvl_points,anchors,centernesses[1]):
            ...
            centerness_AB = centerness_AB.permute(1, 2, 0).reshape(-1).sigmoid()#计算centerness
            nms_pre = cfg.get('nms_pre', -1)
            if nms_pre > 0 and scores.shape[0] > nms_pre:#当bbox太多时,则将得分最高的1000个取出
                max_scores, _ = (scores * 1).max(dim=1)
                _, topk_inds = max_scores.topk(nms_pre)
                bbox_pred_AB = bbox_pred_AB[topk_inds, :]
                scores = scores[topk_inds, :]
                anchor = anchor[topk_inds, :]
                centerness_AB=centerness_AB[topk_inds]
            bboxes = self.anchor_base.bbox_coder.decode(anchor, bbox_pred_AB)#将bbox映射为原图上的坐标
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            mlvl_centerness.append(centerness_AB)
        mlvl_bboxes = torch.cat(mlvl_bboxes)#将所有特征层的bbox融合到同一个四维的张量中
        if rescale:
            mlvl_bboxes /= mlvl_bboxes.new_tensor(scale_factor)
        mlvl_scores = torch.cat(mlvl_scores)#所有特征层的得分融合到一个一维张量中
        padding = mlvl_scores.new_zeros(mlvl_scores.shape[0], 1)#给分类得分补零
        # remind that we set FG labels to [0, num_class-1] since mmdet v2.0
        # BG cat_id: num_class
        mlvl_scores = torch.cat([mlvl_scores, padding], dim=1)
        mlvl_centerness_all = torch.cat(mlvl_centerness)#将所有centerness融合到一个张量中
        # mlvl_centerness_all = torch.ones_like(mlvl_bboxes)[:, 0]
        # mlvl_centerness_all[:len(mlvl_centerness)] = mlvl_centerness
        # mlvl_centerness_all=(mlvl_centerness.repeat(1,10)).squeeze()
        det_bboxes, det_labels = multiclass_nms(#nms输出最终结果
            mlvl_bboxes,
            mlvl_scores,
            cfg.score_thr,
            cfg.nms,
            cfg.max_per_img,
            score_factors=mlvl_centerness_all)
        return det_bboxes, det_labels#返回最终结果
    #计算iou
    ...
    def centerness_target(self, pos_bbox_targets):#计算训练centerness时的标签
